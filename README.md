...
- MedInstruct datasets:
    ...
    2. *MedInstruct-test*: 217 clinician craft free-form instruction evaluation tasks with reference responses generated by `gpt-4`, `gpt-tubro-3.5`, `text-davinci-003` and `claude-2`.
           
    All files is a list of dictionaries in JSON/JSONL format, each dictionary contains the following fields:
    - `instruction`: `str`, the medical task the model should perform. Each of instrcutions in *MedInstruct-52K* and *MedInstruct-test* is unique. The instructions in *MedInstruct-52K* are generated by OpenAI `gpt-4`, while *MedInstruct-test* are clinian-craft.
    ...
    - `difficulty`: `str`,  medical knowledge difficulty level scale from 1-5 evaluated by clinicians following the [scoring system](plots/scoresystem.pdf). This only appears in *MedInstruct-test*.
 


- Instructions:
    1. Clinican-crafted instruction seed set, containing 167 instances. 
    2. All medical instructions generated by OpenAI `gpt-4` with/without filtering.
...
We follows [Alpaca](https://crfm.stanford.edu/2023/03/13/alpaca.html) prompt to fine-tune LLaMA series models and use standard Hugging Face training code.
